...
- [ ] **P10.5** `v10` It shows the opening: captured, quiz, tortured, festival, derby, out — the previously absent festival is now a real pre-heat action in `underground_colosseum.tscn`, not a title card or checklist claim. While vehicle control is already locked, a close press in the cab sightline closes on a whole human silhouette, replaces it with three separately numbered, strapped slabs whose faces retain visible bone, then retracts into the roof before the horn. The ordinary surface derby never builds it. `gore_festival_test.gd` verifies the before/after bodies, one persisted witness event, timing and venue isolation (11/11); the actual production cab was captured before and after and both PNGs were opened (`P:/GameDev/Temp/p10_5_gore_festival_before.png`, `P:/GameDev/Temp/p10_5_gore_festival_after_v2.png`). This line stays open: the larger AP1.3 requirement is torture revealed across later play, not merely the intake procedures already present, so the complete six-beat opening is not yet honest to claim.
