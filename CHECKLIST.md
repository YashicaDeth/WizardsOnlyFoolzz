...
- [x] **AG5.22** September 18 animation recovery: the mouth-held cigarette now rests almost horizontally, low and left of the reticle, instead of pointing vertically through the aiming lane. Hunt enemies and Gore Sandbox targets now own the same articulated body-motion controller as the player: grounded movement opens into distinct walk/run strides with foot lift, shoulder counter-swing, torso/head response and no bind-pose translation; melee visibly anticipates, cuts and recovers; firearm users shoulder both arms, fire, recoil and return while their lower body keeps moving. The finished arm poses now carry their anatomy hitboxes rather than leaving invisible bind-pose targets behind. `smoking_act_test.gd`, `body_motion_test.gd`, `enemy_ai_test.gd` and `gore_sandbox_training_test.gd`; visually checked in `P:/GameDev/Temp/smoking_feel_cigarette_lip_hold.png`, `P:/GameDev/Temp/smoking_feel_cigarette_armed.png` and `P:/GameDev/Temp/gore_sandbox_enemy_mode.png`.
