...
- [ ] **K4.4** A Sin holds *signal*, not a keep — channels taken by argument, hijack or cut (each new faction has a `channel` description now; the contest verbs from `DESIGN/FACTIONS.md` are not built)
...
- [x] ~~**M1.5** The unlock itself is an event the player feels, not a quiet permission change~~
