...
func _ready() -> void:
	...
	var planes := Sephiroth.reachable_planes()
	...
	for plane_id in planes:
		var data := Sephiroth.plane(plane_id)
		check(str(data.get("name", "")) != "", "%s has a real tradition name" % plane_id)

	# --- AV1.3: Da'ath is real data and never a reachable place -------------
	var daath_data := Sephiroth.plane(Sephiroth.DAATH)
	check(str(daath_data.get("name", "")) == "Da'ath", "Da'ath is real data, not an empty result")
	check(not planes.has(Sephiroth.DAATH), "but it is never in the reachable list")
	check(is_inf(Sephiroth.floor_requirement(Sephiroth.DAATH, "see")), "and no floor is even defined for it")

	# --- AV2.1: floors rise with a plane's own order -------------------------
	for floor_name in Sephiroth.FLOOR_ORDER:
		check(is_equal_approx(Sephiroth.floor_requirement("malkuth", floor_name), 0.0), "Malkuth needs no altitude for %s — you are already standing in it" % floor_name)
	var keter_see := Sephiroth.floor_requirement("keter", "see")
	var keter_fight := Sephiroth.floor_requirement("keter", "fight")
	check(keter_see > Sephiroth.floor_requirement("yesod", "see"), "Keter asks more altitude to even see than Yesod does")
	...
	var hellish := Sephiroth.perceive("gevurah", "yetzirah")
	var angelic := Sephiroth.perceive("gevurah", "beriah")
	...
	check(is_equal_approx(Sephiroth.altitude("player"), 0.0), "nothing taken yet, nothing to read (altitude 0)")
	var dose := Substances.take("player", "choir_bloom")
	check(bool(dose.get("ok", false)), "sanity: the door substance was actually taken")
	var after_dose := Sephiroth.altitude("player")
	check(after_dose > 0.0, "taking a door substance actually raises altitude, read back from the event it left")
	check(Sephiroth.has_floor("player", "yesod", "talk"), "and it is enough on its own to talk on a plane one step up")
	...
	last_event["time_msec"] = int(last_event["time_msec"]) - int(Sephiroth.ALTITUDE_HALF_LIFE_MSEC)
	var after_one_half_life := Sephiroth.altitude("player")
	...
	check(Sephiroth.altitude("sitter") > 0.0, "a full meditation hold also reads as real altitude")
	check(Sephiroth.has_floor("sitter", "yesod", "talk"), "sixty seconds of sitting reaches the same floor a dose does, per the design's own parity")

	# --- AV1.4: the petition verb, a real cost to open it -------------------
	var no_seal := Sephiroth.petition("player", "yesod", "", "blood", 50.0)
	check(not bool(no_seal.get("ok", false)), "a petition with no seal is refused outright")

	var too_high := Sephiroth.petition("player", "keter", "a name spoken once", "blood", 50.0)
	check(not bool(too_high.get("ok", false)), "and one aimed above your own altitude is refused before anything is spent")

	var on_daath := Sephiroth.petition("player", Sephiroth.DAATH, "a name", "blood", 50.0)
	...
	var petitioned := Sephiroth.petition("player", "yesod", "a name, spoken with the seal drawn under it", "blood", 80.0)
	...
	var departed := Sephiroth.depart("player", "yesod", "blood", 40.0)
	...
	var still_up := Sephiroth.sustain_or_fail("faller", "malkuth", "talk")
	check(bool(still_up.get("ok", false)), "at altitude 0 on Malkuth (needs 0), sustaining a talk still holds")
	var never_climbed := Sephiroth.sustain_or_fail("faller", "yesod", "talk")
	...
	check(WorldHistory.subject(Sephiroth.DAATH).is_empty(), "Da'ath is never registered — nothing can hold a relationship with a place off the map")

	print("SEPHIROTH_TEST_RESULT failures=", failures.size())
