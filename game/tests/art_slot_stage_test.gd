...
## Cutscene art slots (Greg, 2026-09-24): every slot runs on a placeholder until
...
func _ready() -> void:
	...
	for slot in ArtSlotStage.SLOTS:
		check(stage.has_method("_placeholder_" + str(slot)), "slot %s has a placeholder to fall back on" % slot)
