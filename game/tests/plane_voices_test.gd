...
func _ready() -> void:
	...
	for plane_id in PlaneLadder.reachable_planes():
		check(PlaneVoices.MASK.has(plane_id), "%s has a mask and a jest of its own" % plane_id)
	check(not PlaneVoices.MASK.has(PlaneLadder.DAATH), "Da'ath has no voice here either — nothing off the map gets a face")
	...
	var first := PlaneLadder.petition("player", "yesod", "a name under a seal", "blood", 10.0)
	check(bool(first.get("ok", false)), "the first petition lands")
	PlaneLadder.depart("player", "yesod", "blood", 5.0)
	...
	for _i in 3:
		PlaneLadder.petition("player", "yesod", "a name under a seal", "blood", 10.0)
		PlaneLadder.depart("player", "yesod", "blood", 5.0)
	...
	var arrival := PlaneLadder.petition("player", "yesod", "a name under a seal", "blood", 10.0)
	...
	PlaneLadder.petition("faller", "malkuth", "a seal", "blood", 5.0)
	var faller_after_trip := PlaneVoices.standing("malkuth", "faller")
	PlaneLadder.sustain_or_fail("faller", "chokmah", "talk")
