...
func _ready() -> void:
	...
	var idle: RigidBody3D = _spawn(Vector3(-10, 0.75, 0))
	idle.enabled = true
	idle.throttle = 0.0
	for _frame in 120:
	...
	var burner: RigidBody3D = _spawn(Vector3(10, 0.75, 0))
	burner.enabled = true
	burner.throttle = 1.0
	for _frame in 120:
		await get_tree().physics_frame
	...
	var stalled: RigidBody3D = _spawn(Vector3(0, 0.75, -20))
	...
	var stalled_speed: float = stalled.linear_velocity.length()
