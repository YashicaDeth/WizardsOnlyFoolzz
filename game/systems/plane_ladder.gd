class_name PlaneLadder
extends RefCounted

## Renamed on merge, and the reason is worth keeping.
##
## This arrived from lane 3 as `sephiroth.gd`/`class_name Sephiroth`, and main
## already had a *different* file at that exact path under that exact class
## name: `Sephiroth` is the tree's **geometry** — positions, the tradition's own
## twenty-two paths, names, meanings — written so AR's chart and AV's ladder
## would draw one shape instead of two that drift. This is the **mechanic**:
## what altitude you need before a plane will let you see it, talk to it,
## conjure on it or fight on it.
##
## They are complementary and both wanted, so the merge is a rename rather than
## a choice between them. Nothing was dropped in either direction. `Sephiroth`
## owns where the ten sit and what the paths between them are; `PlaneLadder`
## owns what it costs to get there. The one thing they genuinely duplicate —
## the ten ids and their order — is asserted equal in `plane_ladder_test`, so a
## sephirah added to one and not the other fails a test instead of quietly
## producing a tree with a node the ladder cannot reach.
##
