...
const EYE := Vector3(-0.34, 0.68, -0.06)
...
const GLASS_AT := Vector3(0.0, 0.52, -0.92)
const GLASS_SIZE := Vector2(1.72, 0.86)
...
func _dashboard() -> void:
	...
	_block(Vector3(0, 0.30, -0.62), Vector3(1.78, 0.26, 0.52), DASH)
	_block(Vector3(0, 0.44, -0.86), Vector3(1.78, 0.08, 0.12), DASH.lightened(0.08))
	# The binnacle in front of the driver. Deliberately not a readout — I0 says
	# the hull is read off the car, not off a number, so this is a housing with
	# nothing useful in it.
	_block(Vector3(-0.34, 0.46, -0.58), Vector3(0.42, 0.16, 0.3), DASH.darkened(0.3))
	# The transmission tunnel, which is what makes a cabin feel narrow.
	_block(Vector3(0, 0.14, -0.1), Vector3(0.3, 0.26, 1.1), DASH.darkened(0.15))
...
func _steering_wheel() -> void:
	wheel = Node3D.new()
	wheel.name = "Wheel"
	wheel.position = Vector3(-0.34, 0.40, -0.50)
	...
	for index in segments:
		...
		var seg := _block(Vector3(cos(angle) * 0.19, sin(angle) * 0.19, 0.0), Vector3(0.055, 0.055, 0.05), WHEEL_RIM, wheel)
		seg.rotation.z = angle
	# Two spokes and a boss. Two, not three: it is a wheel somebody welded.
	for spoke in [-0.45, PI + 0.45]:
		var arm := _block(Vector3(cos(spoke) * 0.1, sin(spoke) * 0.1, 0.0), Vector3(0.2, 0.035, 0.035), WHEEL_RIM.lightened(0.06), wheel)
...
func _hands() -> void:
	...
	left_arm.position = Vector3(-0.34, 0.42, -0.48)
	add_child(left_arm)
	...
	gun_arm = Node3D.new()
	gun_arm.name = "GunArm"
	gun_arm.position = Vector3(0.3, 0.52, -0.5)
