	...
	for frame in frame_count:
		...
		match kind:
			...
			"rip":
				...
				sample = ((_noise(frame * 11) * 2.0 - 1.0) * 0.7 + sin(TAU * 140.0 * t) * 0.3) * envelope
			"revenge":
				var envelope := minf(1.0, t * 12.0) * exp(-t * 2.2)
				sample = (sin(TAU * 41.0 * t) * 0.6 + sin(TAU * 41.0 * 1.5 * t) * 0.25 + (_noise(frame) - 0.5) * 0.1) * envelope
			"door":
