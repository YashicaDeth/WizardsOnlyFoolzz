...
func _draw() -> void:
	...
	draw_string(font, plate.position + Vector2(18, 30), subject_name, HORIZONTAL_ALIGNMENT_LEFT, plate.size.x - 36, 19, BONE * Color(1, 1, 1, fade))
	draw_string(font, plate.position + Vector2(18, 50), "LETHAL // %s" % impact_zone.to_upper().replace("_", " "), HORIZONTAL_ALIGNMENT_LEFT, -1, 10, ARTERIAL * Color(1, 1, 1, fade))
