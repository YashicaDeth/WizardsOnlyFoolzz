...
func _draw_wire(rect: Rect2) -> void:
	...
	var traffic_label := "TRAFFIC %s" % wire.activity_band()
	var traffic_width := CellOutzType.width(traffic_label, 9.0, 0.8)
	CellOutzType.draw_condensed(self, Vector2(feed.position.x + feed.size.x * 0.52 - traffic_width * 0.5, feed.position.y + 2), traffic_label, 9.0, MOSS.lerp(HOT, wire.network_activity()), 0.8)
