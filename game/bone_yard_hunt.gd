...
## A9.1 / A9.2 / W1.2. The volume follows the player in steps. Its severity is
## two terms, not one: the number AS4.2 says the storm will read
## (`chaos_magick()`, near zero on a quiet run and climbing with rituals and
## the gods A7 put in the sky) plus the calendar term W1.2 actually asks
## for — a save left running reads as measurably more contaminated before a
## single ritual is ever cast, because the world "gets worse" on its own,
## not only when the player makes it worse.
func _update_air() -> void:
	if air == null or not is_instance_valid(air):
		return
	air.follow(player)
	var worn := clampf(float(WorldClock.day()) / AIR_WORSENING_DAYS, 0.0, 1.0)
	var chaos := clampf(WorldHistory.chaos_magick(), 0.0, 1.0)
	air.set_severity(clampf(worn + chaos * AIR_CHAOS_CONTRIBUTION, 0.0, 1.0))
