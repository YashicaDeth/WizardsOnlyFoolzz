...
func third_person_refusal() -> String:
	if WorldHistory.event_count("melee_body_hit") <= 0:
		return "THERE IS NOTHING BEHIND YOUR EYES YET. SWING AT SOMEBODY FIRST."
	return "THE RELAY HAS NOTHING TO SHOW YOU. PUT DOWN SOMEONE WHO MATTERS."
...
func _update_camera() -> void:
	...
	# AX5.2. A corridor, a tunnel, a vat room: the oldest unsolved problem in
	# third person is what the chase camera does when there is no room for it.
	# `collision_safe_camera()` just answered "where", pulling `tp_position` in
	# to whatever clearance actually exists; `clearance` is "how much of a shot
	# that still leaves" — the fraction of the requested distance it kept. The
	# blend target is `third_person` scaled by that fraction rather than a flat
	# 1.0, so the implant cedes back to your own eyes exactly as fast as the
	# walls close in, and never leaves a chase camera sitting inside geometry
	# or jammed into your own shoulder blades. Open ground behind the player
	# reads as a clearance near 1 and changes nothing.
	var clearance := HUNTER_MOTOR.third_person_clearance_blend(focus, camera_position, tp_position)
	...
	# `perspective_blend` sliding toward whatever `third_person` (and now the
	# room around the player) asks for. This used to be an
	# `if third_person: ... else: ...` that snapped the camera's position and
	# FOV on the same frame the key was pressed, which is exactly the hard cut
	# Rule 3 names as a bug. Wide inside your own head, ordinary outside it —
	# the change between the two is the reward, and a reward you can watch
	# happen reads better than one you only notice has already happened.
	perspective_blend = move_toward(perspective_blend, (1.0 if third_person else 0.0) * clearance, PERSPECTIVE_BLEND_RATE * get_physics_process_delta_time())
