...
Project skills live in `.claude/skills/` (Agent Skills format; each is a folder with a `SKILL.md`). Start with `goal` (the current goal and the ordered work queue; `/goal`). Read the matching one before working: `wof-lane-hygiene` before any edit or commit, `wof-verify-by-looking` for anything visual, `wof-wire-before-polish` before improving a system, `wof-agent-brief` when dispatching work to another agent, `wof-combat-fx` for any fighting visual or sound. The `godot-*` skills (vendored from gamedev-skills/awesome-gamedev-agent-skills, pinned; see `DESIGN/ASSET_TOOLS.md`) cover general Godot 4.7 practice; the `wof-*` skills win where they differ. Generated assets come in only through `tools/import_generated_asset.py`.
